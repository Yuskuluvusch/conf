...
{
    public const ERROR_STEP_REQUIRED = 'ERROR_STEP_REQUIRED';
    public const ERROR_STEP_FILEUPLOAD_REQUIRED = 'ERROR_STEP_FILEUPLOAD_REQUIRED';
    public const ERROR_STEP_OCCURED = 'ERROR_STEP_OCCURED';
    public const ERROR_PRICELIST_VALUES = 'ERROR_PRICELIST_VALUES';
    public const ERROR_PRICELIST_VALUES_FOR = 'ERROR_PRICELIST_VALUES_FOR';
    public const ERROR_MAXOPTIONS_REACHED = 'ERROR_MAXOPTIONS_REACHED';
    public const ERROR_MINOPTIONS_REACHED = 'ERROR_MINOPTIONS_REACHED';
    public const ERROR_MAXQTY_REACHED = 'ERROR_MAXQTY_REACHED';
    public const ERROR_STEPQTY_REACHED = 'ERROR_STEPQTY_REACHED';
    public const ERROR_FORMULA = 'ERROR_FORMULA';
    public const ERROR_FORMULA_VALUE = 'ERROR_FORMULA_VALUE';
    public const ERROR_FORMULA_VALUE_PREVIOUS = 'ERROR_FORMULA_VALUE_PREVIOUS';
    public const ERROR_FORMULA_VALUE_EMPTY = 'ERROR_FORMULA_VALUE_EMPTY';
    public const ERROR_FORMULA_METHOD = 'ERROR_FORMULA_METHOD';
    public const ERROR_FORMULA_ADAPTER = 'ERROR_FORMULA_ADAPTER';
    public const ERROR_FORMULA_DIVISION_BY_ZERO = 'Division by zero';
    public const ERROR_MIN_REQUIRED = 'ERROR_MIN_REQUIRED';

    public const EMAIL_ORDER_CONF_TPL = 'order_conf';

    public const MAX_FLOAT_PRECISION_MULTIPLIER = 1000000;

    protected static $routes = [
        'module-configurator-attachment' => [
            'controller' => 'attachment',
            'rule' => 'configurator-attachment/{token}',
            'keywords' => [
                'token' => ['regexp' => '[a-z0-9]+', 'param' => 'token'],
            ],
            'params' => [
                'fc' => 'module',
                'module' => 'configurator',
    ...
    protected $hook_list = [
        ...
        'actionObjectAttributeUpdateAfter',
        'actionObjectAttributeAddAfter',
        'actionAdminAttributesGroupsFormModifier',
        'actionProductUpdate',
    ...
    protected static $added_in_cart = false;
    ...
    public function install($keep = false)
    {
        ...
        Configuration::updateValue('CONFIGURATOR_DISPLAY_SHARING_BUTTON', 0);
        Configuration::updateValue('CONFIGURATOR_DISPLAY_UNIT_PRICE', 0);
        Configuration::updateValue('CONFIGURATOR_REDIRECT_AFTER_CART', 0);
        Configuration::updateValue('CONFIGURATOR_HIDE_EMPTY_TABS', 0);

        $lang_values = [];
        foreach (Language::getLanguages() as $lang) {
            $lang_values['CUSTOMIZATION_FIELD'][$lang['id_lang']] = $this->l('Configurator', false, $lang['locale']);
            $lang_values['Configurator'][$lang['id_lang']] = $this->l('Product configurator', false, $lang['locale']);
            $lang_values['ConfiguratorSteps'][$lang['id_lang']] = $this->l('Configurator steps', false, $lang['locale']);
            $lang_values['ConfiguratorTabs'][$lang['id_lang']] = $this->l('Configurator tabs', false, $lang['locale']);
            $lang_values['ConfiguratorTools'][$lang['id_lang']] = $this->l('Configurator tools', false, $lang['locale']);
        }

        $this->setConfigurationLang('CONFIGURATOR_CUSTOMIZATION_FIELD_NAME', $lang_values['CUSTOMIZATION_FIELD']);
        ...
        $install = $install_attribute_group
            && $this->_installModuleTab('AdminConfigurator', $lang_values['Configurator'])
            && $this->_installModuleTab('AdminConfiguratorSteps', $lang_values['ConfiguratorSteps'], 0)
            && $this->_installModuleTab('AdminConfiguratorTabs', $lang_values['ConfiguratorTabs'], 0)
            && $this->_installModuleTab('AdminConfiguratorTools', $lang_values['ConfiguratorTools'], 0)
            && $this->_installMeta()
            && parent::install()
            && $this->_installHooks();
    ...
    private function _installHooks()
    {
        ...
        foreach ($this->hook_list as $hook_name) {
            $result &= $this->registerHook($hook_name);
        }
    ...
    public function uninstall($keep = false)
    {
        ...
        Configuration::deleteByName('CONFIGURATOR_DISPLAY_SHARING_BUTTON');
        Configuration::deleteByName('CONFIGURATOR_DISPLAY_UNIT_PRICE');
        Configuration::deleteByName('CONFIGURATOR_REDIRECT_AFTER_CART');
        Configuration::updateValue('CONFIGURATOR_HIDE_EMPTY_TABS', 0);
    ...
    protected function getConfigForm()
    {
        ...
        $form = [
            'form' => [
                ...
                'tabs' => [
                    'configurator' => '<i class="icon-cog"></i> ' . $this->l('Configurator'),
                ],
                'input' => [
                    [
                        'tab' => 'configurator',
                    ...
                    [
                        'tab' => 'configurator',
                        'type' => 'switch',
                        'label' => $this->l('Hide empty tabs'),
                        'name' => 'CONFIGURATOR_HIDE_EMPTY_TABS',
                        'hint' => $this->l(
                            'Hide tab if it contains no visible steps'
                        ),
                        'is_bool' => true,
                        'values' => $booleanValues,
                    ],
                    ...
                    [
                        'tab' => 'configurator',
                        'type' => 'switch',
                        'label' => $this->l('Display sharing button'),
                        'name' => 'CONFIGURATOR_DISPLAY_SHARING_BUTTON',
                        'is_bool' => true,
                        'values' => $booleanValues,
                    ],
                    [
                        'tab' => 'configurator',
                        'type' => 'switch',
                        'label' => $this->l('Display unit price if step quantity exists'),
                        'name' => 'CONFIGURATOR_DISPLAY_UNIT_PRICE',
                        'is_bool' => true,
                        'values' => $booleanValues,
                    ],
                    [
                        'tab' => 'configurator',
                        'type' => 'switch',
                        'label' => $this->l('Redirect to the product after adding the product in cart?'),
                        'name' => 'CONFIGURATOR_REDIRECT_AFTER_CART',
                        'is_bool' => true,
                        'values' => $booleanValues,
                    ],
                ],
        ...
        Hook::exec('configuratorGetConfigForm', ['form' => &$form]);
    ...
    protected function getConfigFormValues()
    {
        $values = [
            ...
            'CONFIGURATOR_DISPLAY_SHARING_BUTTON' => Configuration::get('CONFIGURATOR_DISPLAY_SHARING_BUTTON'),
            'CONFIGURATOR_DISPLAY_UNIT_PRICE' => Configuration::get('CONFIGURATOR_DISPLAY_UNIT_PRICE'),
            'CONFIGURATOR_REDIRECT_AFTER_CART' => Configuration::get('CONFIGURATOR_REDIRECT_AFTER_CART'),
            'CONFIGURATOR_HIDE_EMPTY_TABS' => Configuration::get('CONFIGURATOR_HIDE_EMPTY_TABS'),
        ];

        Hook::exec('configuratorGetConfigFormValues', ['values' => &$values]);
    ...
    public function hookBackOfficeHeader()
    {
        ...
        switch ($controller_name) {
            ...
            case 'AdminConfigurator':
                $this->context->controller->addCSS($this->_path . 'views/css/back.css');
                $this->context->controller->addJs($this->_path . 'views/js/admin-new-theme.js');
                break;
            case 'AdminOrders':
            case 'AdminCarts':
            case 'AdminCustomers':
    ...
    /**
     * hookHeader is deprecated in Prestashop 8, so we use hookDisplayHeader
     */
    ...
    {
        if (Tools::getValue('controller') !== 'product' && !Product::isConfigurated(Tools::getValue('id_product', 0))) {
            return;
        }
        ...
        $js_files = [
            ...
            101 => [
                ...
                'modules/configurator/views/js/front/modules/fancybox/fancybox.js',
            ],
            102 => [
    ...
    public function hookModuleRoutes()
    {
         ...
         */
        return array_merge(self::$routes, ['product_rule' => $override_product_rule]);
    }
    ...
    public function getErrorTranslation($key)
    {
        ...
        Hook::exec('configuratorActionGetErrorTranslation', [
    ...
    public function hookActionObjectAttributeUpdateAfter($params)
    {
        ...
        $attribute->texture_image = (bool) Tools::getValue('texture_image');
    ...
    public function hookActionProductUpdate($params)
    {
        ...
        $configurator->use_combination = Tools::getValue('use_combination');

        // CONFIGURATOR HOOK
        Hook::exec('configuratorActionProductUpdate', ['configurator' => &$configurator]);
    ...
    {
        $product = new Product($params['id_product'], false, Context::getContext()->language->id);
        if (!ConfiguratorModel::isConfiguratedProduct((int) $product->id)) {
            if (Validate::isLoadedObject($product)) {
                ...
                $this->smarty->assign(['configurator' => $configurator]);
            } else {
                $product = false;
            }
            ...
            // CONFIGURATOR HOOK
            $HOOK_CONFIGURATOR_DISPLAY_ADMIN_PRODUCTS_EXTRA = Hook::exec('configuratorDisplayAdminProductsExtra', [
            ...
            $this->smarty->assign([
                ...
                'languages' => Language::getLanguages(),
                'languages_json' => json_encode(Language::getLanguages()),
                'HOOK_CONFIGURATOR_DISPLAY_ADMIN_PRODUCTS_EXTRA' => $HOOK_CONFIGURATOR_DISPLAY_ADMIN_PRODUCTS_EXTRA,
                'show_configurator' => Tools::getIsset('show_configurator'),
            ]);
    ...
    public function hookDisplayCustomization($params)
    {
        ...
        if (isset($cart_detail[0]) && Validate::isLoadedObject($cart_detail[0])) {
            ...
            if ($cart_detail[0]->visual_rendering) {
                $this->context->smarty->assign('visual_rendering', $cart_detail[0]->visual_rendering);
                $return .= $this->display(__FILE__, 'display-customization-visual-rendering.tpl');
            }
            // Find id_cart
            ...
            $linkParams = [
                'configurator_update' => $cart_detail[0]->id,
                'id_cart' => $id_cart,
                'hash' => $hash,
            ];
            $link = Context::getContext()->link->getProductLink(
                (int) $params['customization']['id_product'],
                null,
                null,
                null,
                null,
                null,
                $params['customization']['id_product_attribute'],
                false,
                false,
                false,
                $linkParams
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        $this->smarty->assign([
            ...
            'products' => $products,
            'attachment_link' => $this->context->link->getModuleLink('configurator', 'attachment', ['token' => '']),
        ]);
    ...
    {
        return Configuration::getConfigInMultipleLangs($key);
    }

    public function setConfigurationLang($key = '', $values = [])
    {
