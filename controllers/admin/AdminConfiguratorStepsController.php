...
{
    public const ADMIN_CONFIGURATOR_CONTROLLER = 'AdminConfigurator';

    private $_id_configurator;

    public $available_tabs = [];
    public $conditions_choices = [];
    public $filters_choices = [];
    public $find_division_step = false;
    public $id_tab_selected = 'ConfigurationStep';

    public static $types = [];
    public static $filter_types = [];
    public static $price_list_types = [];
    public static $price_list_display = [];
    public static $impact_types = [];
    public static $price_calculation_types = [];
    public static $price_calculation_default;
    ...
    public function __construct()
    {
        ...
        $this->available_tabs['ConfigurationStepAdvanced'] = $this->l('Advanced configuration');
        ...
        self::$filter_types[ConfiguratorStepFilterAbstract::TYPE_FILTER_ATTRIBUTES] = $this->l('Attributes');

        // Price list types
        self::$price_list_types = [
    ...
    public function postProcess()
    {
        ...
        // CONFIGURATOR HOOK
        Hook::exec('configuratorAdminActionStepsControllerAfterPostProcess', [
            'controller' => &$this,
        ]);
    }
    ...
    public function setMedia($isNewTheme = false)
    {
        ...
        $this->addJS([
             ...
             * jQuery Plugin
             */
            _MODULE_DIR_ . $this->module->name . '/views/js/typewatch.js',
            /*
             * Module Scripts
             */
        ...
        // CONFIGURATOR HOOK
        Hook::exec('configuratorAdminActionStepsControllerSetMedia', [
            'controller' => &$this,
        ]);
    }
    ...
    public function initPageHeaderToolbar()
    {
        ...
        if (Validate::isLoadedObject($product)) {
            ...
            $this->page_header_toolbar_btn['preview_product'] = [
        ...
        if ($sfContainer !== null) {
            ...
            $url .= '&show_configurator#tab-hooks';

            $this->page_header_toolbar_btn['back_product'] = [
                'href' => $url,
        ...
        $this->page_header_toolbar_btn['back_previous_page'] = [
            'href' => $url,
    ...
    public function processSave()
    {
        ...
        // CONFIGURATOR HOOK
        Hook::exec('configuratorAdminStepsControllerProcessSave', [
            'controller' => &$this,
    ...
    private function saveOption($step, $configurator_step_option)
    {
        if (Validate::isLoadedObject($configurator_step_option)) {
            ...
            // CONFIGURATOR HOOK
            Hook::exec('configuratorAdminActionStepsBeforeSaveOption', [
            ...
            $configurator_step_option->content = [];
            $configurator_step_option->placeholder = [];
            foreach ($this->getLanguages() as $language) {
                $configurator_step_option->content[(int) $language['id_lang']] = Tools::getValue(
                    'content_' . $input_id . '_' . (int) $language['id_lang']
                );
                $configurator_step_option->placeholder[(int) $language['id_lang']] = Tools::getValue(
                    'placeholder_' . $input_id . '_' . (int) $language['id_lang']
                );
            }
            ...
            $configurator_step_option->option_min_qty = Tools::getValue('option_min_qty_' . $input_id);
            $configurator_step_option->option_max_qty = Tools::getValue('option_max_qty_' . $input_id);
            ...
            $configurator_step_option->is_special_text = Tools::getValue('is_special_text_' . $input_id);
            $configurator_step_option->special_text_template = Tools::getValue('special_text_template_' . $input_id);
            ...
            $configurator_step_option->delivery_impact = Tools::getValue('delivery_impact_' . $input_id);
            ...
            if ($impact = Tools::getValue('impact_' . $input_id)) {
                ...
                $configurator_step_option->price_list_coeff = Tools::getValue('price_list_coeff_' . $input_id);
                ...
                if ($impact_value_period_tmp) {
                    ...
                    if (is_array($impact_value_period_tmp)) {
                        foreach ($impact_value_period_tmp as $i => $period_value) {
                            if (isset($period_value['id'], $period_value['values'])) {
                                if (isset($period_value['values']['date_start'], $period_value['values']['date_end'], $period_value['values']['specific_value'])
                                ) {
                                    if (Validate::isDate($period_value['values']['date_start'])
                                        && Validate::isDate($period_value['values']['date_end'])
                                    ) {
                                        $impact_value_period[(int) $i] = [
                                            'id' => (int) $i,
                                            'values' => [
                ...
                switch ($configurator_step_option->impact_type) {
                    case ConfiguratorStepOptionAbstract::IMPACT_TYPE_PRICELIST:
                        ...
                        if (!$file['error'] && $pricelist_helper->load($file)) {
                            $configurator_step_option->price_list = json_encode(
            ...
            if ($divisions && isset($divisions[$configurator_step_option->id])) {
                $configurator_step_option->id_configurator_step_option_division =
                    (int) $divisions[$configurator_step_option->id];
            } else {
                $configurator_step_option->id_configurator_step_option_division = 0;
            }

            // CONFIGURATOR HOOK
            Hook::exec('configuratorAdminActionStepsAfterSaveOption', [
                'controller' => &$this,
    ...
    public function renderForm()
    {
        ...
        foreach (array_keys($this->available_tabs) as $id) {
            if (method_exists($this, 'initForm' . $id)) {
                ...
                Hook::exec('configuratorAdminActionStepsControllerInitForm', [
                    'id' => $id,
                    'controller' => &$this,
    ...
    public function processGetPreviousSteps()
    {
        if (Validate::isLoadedObject($this->object)) {
            ...
            if (Module::isInstalled('dm_advancedformula') && Module::isEnabled('dm_advancedformula')) {
                $this->conditions_choices['block_formula'] = [
                    'name' => $this->l('Formula'),
                    'groups' => [
                        [
                            'class' => 'col-lg-8',
            ...
            foreach ($previousSteps as $configuratorStep) {
                ...
                $options_step[$configuratorStep->id] = [
                ...
                foreach ($options as $configurator_step_option) {
                    if (Validate::isLoadedObject($configurator_step_option)) {
                        $options_list[$configurator_step_option->id] = [
                ...
                if (!empty($options_list)) {
                    ...
                    $this->conditions_choices['block_option']['groups'][1]['selects'][] = [
                        'params' => [
            ...
            if (!empty($options_step)) {
                ...
                $this->conditions_choices['block_option']['groups'][0]['selects'][] = [
                    'params' => [
                        'class' => 'select_step',
                    ],
    ...
    private function getFiltersChoices($previousSteps)
    {
        ...
        foreach ($attributes as $attribute) {
            $attributes_options[$attribute['id_attribute_group']] = $attribute['name'];
        }
        asort($attributes_options);
        ...
        $this->filters_choices['attributes_options'] = $attributes_options;
    ...
    public function initModal()
    {
        if (!$this->ajax
            ...
            // CONFIGURATOR HOOK
            Hook::exec('configuratorAdminActionStepsControllerInitModal', [
                'controller' => &$this,
            ]);
        }
    }
    ...
    public function initFormConfigurationStepAdvanced(ConfiguratorStepAbstract $configurator_step)
    {
        $data = $this->createTemplate($this->tpl_form);
        // Languages
        $data->assign('languages', $this->_languages);
        ...
        // Rendering
        $this->tpl_form_vars['tabs']['ConfigurationStepAdvanced']['form_content_html'] = $data->fetch();
    }
    ...
    public function initFormAttributes(ConfiguratorStepAbstract $configurator_step)
    {
        ...
        $data->assign('special_text_option_template_list', array_diff(
            scandir(_PS_MODULE_DIR_ . 'configurator/views/templates/front/elements/attributes/special_text'),
            ['..', '.', 'index.php']
        ));
        // Rendering
    ...
    public function getOptionHtml($configurator_step_option)
    {
        ...
        // CONFIGURATOR HOOK
        $HOOK_CONFIGURATOR_DISPLAY_ADMIN_OPTION_SETTINGS = Hook::exec('configuratorAdminDisplayOptionSettings', [
        ...
        array_unshift($tax_impact_types, [
        ...
        $tpl->assign([
            ...
            'special_text_option_template_list' => array_diff(
                scandir(_PS_MODULE_DIR_ . 'configurator/views/templates/front/elements/attributes/special_text'),
                ['..', '.', 'index.php']
            ),
    ...
    public function getJsonConditions($type, $foreignkey)
    {
        ...
        return json_encode($jsonConditions);
    }
    ...
    public function getJsonFilters($type, $foreignkey)
    {
        ...
        return json_encode($jsonFilters);
    }
    ...
    public function getListAttributes($options)
    {
        ...
        // Custom list links
        $helper->actions = ['editImpactPrice', 'selectedByDefault'];
    ...
    private function downloadPriceList()
    {
        ...
        if (Tools::getIsset('example')) {
            if ((int) Tools::getValue('example') === 1) {
                ...
                $array_to_csv = [
                    ['', 100, 200, 300, 400, 500],
                    [5, 100, 190, 270, 340, 400],
                    [10, 110, 200, 280, 350, 410],
                    [15, 130, 220, 300, 370, 430],
                    [20, 150, 240, 320, 390, 450],
                ];
            }
        } else {
            ...
            if (!empty($configurator_step->price_list)) {
                ...
                foreach ($pricelist as $key => $pricelist_line) {
                    ...
                    $array_to_csv[] = array_merge([$key], (array) $pricelist_line);
                }
            }
