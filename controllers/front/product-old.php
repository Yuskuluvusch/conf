...
class ConfiguratorProductModuleFrontController extends ProductController
{
    ...
    public function setMedia2()
    {
        ...
        $this->addJs(_MODULE_DIR_ . 'configurator/views/js/front/modules/fancybox/fancybox.js');
    ...
    public function init()
    {
        ...
        switch (Tools::getValue('action')) {
            case 'obtainSharingLink':
                $this->obtainSharingLink((int) Tools::getValue('cartDetailId'));
                break;
            case 'checkProductAttributeQuantity':
                $idProductAttribute = (int) Tools::getValue('id_product_attribute');
                $this->checkProductAttributeQuantity($idProductAttribute);
                break;
        }
        ...
        if ($this->use_module_configurator && !$this->updateCart) {
            ...
            $this->configuratorCartDetail->id_product = $this->configurator->id_product;
            $this->configuratorCartDetail->setProduct();
        } elseif ($this->use_module_configurator && $this->updateCart) {
            $need_reset = true;

            foreach ($this->configuratorCartDetail->getDetail() as $key => $step) {
                if (is_integer($key)) {
                    $need_reset = false;
                }
            }
            ...
            if ($need_reset) {
                $this->configuratorCartDetail->setDetail('');
                $this->configuratorCartDetail->save();
                $this->configuratorCartDetail->deleteAttachements();
            }
        }
    }
    ...
    public function initContent()
    {
        ...
        if ($this->use_module_configurator) {
            ...
            // Display modal added in cart
            $display_modal_added_in_cart = Context::getContext()->cookie->__get('display_modal_added_in_cart');
            $old_configurator_cart_detail = null;
            if ($display_modal_added_in_cart) {
                Context::getContext()->cookie->__unset('display_modal_added_in_cart');
                $old_id_customization = Context::getContext()->cookie->__get('display_modal_added_in_cart_customization');
                Context::getContext()->cookie->__unset('display_modal_added_in_cart_customization');
                $old_configurator_cart_detail = ConfiguratorCartDetailModel::getOneByIdProductAndIdCustomization($this->product->id, $old_id_customization);
            }
            ...
            $datas = [
                ...
                'HIDE_EMPTY_TABS' => Configuration::get('CONFIGURATOR_HIDE_EMPTY_TABS'),
                ...
                'display_modal_added_in_cart' => $display_modal_added_in_cart,
                'configuratorCurrentUrlGetParams' => $this->getCurrentUrlGetParams(),
                'old_cart_detail' => $old_configurator_cart_detail,
            ];

            Hook::exec(
                'actionConfiguratorBuildHtmlBefore',
                [
                    'datas' => &$datas,
                    'controller' => &$this,
                ]
            ...
            $configuratorHtml = $this->getConfiguratorHtml($datas);
            $configuratorLeftColumnHtml = $this->getConfiguratorLeftColumnHtml($datas);

            Hook::exec(
                'actionConfiguratorBuildHtmlAfter',
                [
                    'datas' => &$datas,
                    'configuratorHtml' => &$configuratorHtml,
                    'configuratorLeftColumnHtml' => &$configuratorLeftColumnHtml,
                    'controller' => &$this,
    ...
    public function postProcess()
    {
        ...
        if ($this->use_module_configurator) {
            ...
            if ($this->ajax && $action !== 'initContent') {
                $return = [
                ...
                $return_optional = [];
                if (Hook::exec('configuratorActionCartDetailReturn', [
                    'cart_detail' => &$this->configuratorCartDetail,
                    'return_optional' => &$return_optional,
                ])) {
                    foreach ($return_optional as $key => $module_return) {
                        $return[$key] = $module_return;
                    }
                }
    ...
    public function setStepsInfos()
    {
        ...
        foreach ($this->configurator->steps as $step) {
            ...
            if ($step->type !== ConfiguratorStepAbstract::TYPE_STEP_UPLOAD && $step->type !== ConfiguratorStepAbstract::TYPE_STEP_DESIGNER && isset($detail[$step->id]) && count($detail[$step->id]['options']) <= 0) {
                $this->configuratorCartDetail->steps_infos[$step->id] = $this->module->l('', 'product');
            }
        }
    }
    ...
    public function displayAjaxUpload()
    {
        ...
        foreach ($files as &$file) {
            ...
            $file['delete_url'] = $this->context->link->getModuleLink(
                'configurator',
                'attachment',
                ['token' => $attachment->token, 'action' => 'delete']
            );
        ...
        $this->ajaxDie(json_encode([$file_uploader->getName() => $files]));
    }
...
public function getCurrentConfiguratorCartDetail()
{
    ...
    if (!Validate::isLoadedObject($configuratorCartDetail)) {
        $idCustomer = (int) $this->context->customer->id; // Obtener el ID del cliente si está registrado
        $idGuest = (int) $this->context->cart->id_guest; // Obtener el ID del invitado si no hay cliente
        $idUser = $idCustomer > 0 ? $idCustomer : $idGuest; // Priorizar cliente sobre invitado
        ...
        // Añadir el detalle del carrito si no existe
        if (!Validate::isLoadedObject($configuratorCartDetail) || !isset($configuratorCartDetail->id)) {
            ...
            $configuratorCartDetail->id_guest = $idGuest;
            $configuratorCartDetail->id_customer = $idCustomer;
            $configuratorCartDetail->id_product = (int) $this->configurator_product->id;
            $configuratorCartDetail->id_product_attribute = (int) $this->configurator_product->cache_default_attribute;
            $configuratorCartDetail->product = $this->configurator_product;
            $configuratorCartDetail->add();

            // Limpiar caché de precios para evitar conflictos
            Product::flushPriceCache();
        }

        // Si no es AJAX, reinicializar el detalle del carrito
    ...
    // Compartir detalles del carrito si es necesario
    $configurator_cart_detail_shared = new ConfiguratorCartDetailModel((int) Tools::getValue('configurator_shared', 0));
    if (Validate::isLoadedObject($configurator_cart_detail_shared)) {
        $configuratorCartDetail->setDetail($configurator_cart_detail_shared->getDetail());
        $configuratorCartDetail->save();
    }
    ...
    public function getConfiguratorLeftColumnHtml($datas)
    {
        $tpl = $this->context->smarty->createTemplate(
            $this->getTemplatePath('elements/left_column_17.tpl'),
            $this->context->smarty
        );
    ...
    private function getPreviewParams()
    {
        ...
        $ipa = ($this->configurator->use_combination ? $this->configuratorCartDetail->id_product_attribute : null);
        ...
        if (isset($specificPrice['reduction_type'], $specificPrice['reduction']) && $specificPrice['reduction_type'] === 'percentage') {
        ...
        // Needed for datedelivery module
        if (!isset($context->smarty->tpl_vars['product'])) {
            $productArray = [
            ...
            $this->context->smarty->assign('product', $productArray);
        } else {
            $productArray = $context->smarty->tpl_vars['product']->value;
        }
        ...
        return [
            ...
            'displaySharingButton' => (bool) Configuration::get('CONFIGURATOR_DISPLAY_SHARING_BUTTON'),
            'displayUnitPrice' => (bool) Configuration::get('CONFIGURATOR_DISPLAY_UNIT_PRICE'),
            'configuratorStepQuantities' => $this->configuratorCartDetail->getStepQuantities(),
            'configuratorProductAttribute' => Tools::getValue('configurator_product_attribute', $this->configuratorCartDetail->id_product_attribute),
        ];
    }

    public function getCurrentUrlGetParams()
    {
        if ($this->updateCart) {
            return '?configurator_update=' . Tools::getValue('configurator_update')
    ...
    public function checkRequiredSteps()
    {
        ...
        foreach ($required_steps as $step) {
            ...
            if ($this->configuratorCartDetail->stepCanBeAdded($step)) {
                ...
                Hook::exec('configuratorCheckRequiredStep', [
                    'found_one_selected' => &$found_one_selected,
                ...
                if ($found_one_selected === null) {
                    ...
                    if ($step->isType(ConfiguratorStepAbstract::TYPE_STEP_UPLOAD)
                        ...
                        $stepOptions = ConfiguratorStepOptionAbstract::getByIdConfiguratorStep((int) $step->id);
                        foreach ($stepOptions as $stepOption) {
                            if (!$step->use_input) {
                                $found_one_selected = $this->configuratorCartDetail->foundOption((int) $stepOption->id)
                                    ? true
                                    : $found_one_selected;
                            } elseif ($step->use_input && $this->configuratorCartDetail->existOption((int) $stepOption->id)) {
                                // vérifier si id_option est dans configurator_cart_detail
                                $found_one_selected = $found_one_selected
                                    && $this->configuratorCartDetail->foundOption((int) $stepOption->id, false, true, (int) $step->accept_zero_value);
                            }
                        }
                    }
                }

                if (!$found_one_selected && !$this->configuratorCartDetail->isSharedStepCompleted($step->id)) {
                    $result = false;
    ...
    public function checkPricelistValuesSteps()
    {
        ...
        foreach ($detail as $step_detail) {
            ...
            if (($step->use_input && $step->required) || !empty($step->price_list)) {
                $required = $step->required;
                foreach ($step_detail['options'] as $option) {
                    if (!empty($option['value']) || ($step->accept_zero_value && $option['value'] === '0')) {
                        /**
    ...
    public function processSubmitCart()
    {
        if ($this->checkRequiredSteps()
            ...
            $idProductAttributeOld = 0;
            if ($this->configurator->use_combination) {
                $idProductAttribute = (int) Tools::getValue('configurator_product_attribute', 0);
                $idProductAttributeOld = (int) $this->configuratorCartDetail->id_product_attribute;
            }
            ...
            $this->configuratorCartDetail->id_product_attribute = $idProductAttribute;
            $this->configuratorCartDetail->save();
            ...
            $params = [
                'add' => '1',
                'id_product' => (int) $this->configuratorCartDetail->product->id,
            ...
            if ($this->updateCart) {
                ...
                if ($idProductAttribute !== $idProductAttributeOld) {
                    $idCustomizationOld = $this->configuratorCartDetail->id_customization;
                    $customization->id = null;
                    $customization->quantity = 0;
                    $customization->save();
                    $this->configuratorCartDetail->id_customization = $customization->id;
                    $this->configuratorCartDetail->save();

                    Db::getInstance()->execute(
                        'UPDATE `' . _DB_PREFIX_ . 'customized_data`
                            SET `id_customization` = ' . (int) $customization->id . '
                            WHERE `id_customization` = ' . (int) $idCustomizationOld
                    );
                    ...
                    $params['id_customization'] = $customization->id;
                }
                $params['qty'] = (int) $qty;

                if ($qty < 0) {
                    $params['qty'] = (int) abs($qty);
                    $params['op'] = 'down';
                } elseif ($qty === 0) {
                    $params = [];
                }
            ...
            // CONFIGURATOR HOOK
            Hook::exec('beforeModuleConfiguratorProcessSubmitCart', [
                'controller' => $this,
            ]);
            ...
            Tools::redirectLink($this->context->link->getPageLink(
                ...
                true,
                null,
                $paramsStr . 'configurator=1&token=' . Tools::getToken(false),
                false
            ));
        } else {
            // CONFIGURATOR HOOK
    ...
    private function handleAdd($operation)
    {
        ...
        if (Validate::isLoadedObject($step) && Validate::isLoadedObject($option)) {
            $extra = ['selected' => true, 'multiple' => $step->multiple];

            if (
                $step->isValidPricelistValue((float) $values['value'], $values['dimension'])
    ...
    public function processCartDetail()
    {
        ...
        $this->configuratorCartDetail->id_product_attribute = Tools::getValue('configurator_product_attribute');
    ...
    public function processStep($step, $extras = [])
    {
        $detail = $this->configuratorCartDetail->getDetail();
    ...
    public function processAddOptions($step)
    {
        ...
        $attributes = [];
        if ($step->use_combination_as_default_value) {
        ...
        foreach ($step->options as $option) {
            if (Validate::isLoadedObject($option)) {
                ...
                $this->configuratorCartDetail->addOption((int) $step->id, $option, [
    ...
    public function obtainSharingLink($cartDetailId = 0)
    {
        $configuratorCartDetail = new ConfiguratorCartDetailModel($cartDetailId);

        if (Validate::isLoadedObject($configuratorCartDetail)) {
            $configuratorCartDetailDuplicated = $configuratorCartDetail->duplicate();
            $configuratorCartDetailDuplicated->is_shared = 1;
            $configuratorCartDetailDuplicated->save();

            $link = Context::getContext()->link->getProductLink((int) $configuratorCartDetailDuplicated->id_product);
            $link .= '?configurator_shared=' . $configuratorCartDetailDuplicated->id;
