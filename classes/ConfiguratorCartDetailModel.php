...
if (!class_exists('ConfiguratorCartDetailModel')) {
    ...
    {
        public const PRODUCT_CART_DETAIL_MAX_QTY = 10000;
        ...
        public static $definition = [
            ...
            'fields' => [
                ...
                'id_order' => ['type' => self::TYPE_INT, 'validate' => 'isNullOrUnsignedId'],
                'id_order_detail' => ['type' => self::TYPE_INT, 'validate' => 'isNullOrUnsignedId'],
                'detail' => ['type' => self::TYPE_STRING],
                'added_in_cart' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
                'price' => ['type' => self::TYPE_FLOAT, 'validate' => 'isPrice'],
                'weight' => ['type' => self::TYPE_FLOAT, 'validate' => 'isUnsignedFloat'],
                'delivery_impact' => ['type' => self::TYPE_INT, 'validate' => 'isInt'],
                ...
                'is_shared' => ['type' => self::TYPE_BOOL],
                // Module Viewer 2D
                'visual_rendering' => ['type' => self::TYPE_STRING],
            ],
        ];
        ...
        public function checkOptionValueCondition($id_step, ConfiguratorStepDisplayConditionModel $condition)
        {
            ...
            if (isset($detail[(int) $id_step]['options'][(int) $condition->value]['value'])) {
                ...
                if ((float) $condition->min > 0
                    && (float) $condition->max === 0
                    && (float) $stepValue >= (float) $condition->min
                ) {
                    return true;
                } elseif ((float) $condition->min === 0
                    && (float) $condition->max > 0
                ...
                if ($stepValue >= 0 && (float) $condition->min === 0 && (float) $condition->max === 0) {
                    return true;
                }
            }
                ...
                foreach ($detail as $step_detail) {
                    if (isset($step_detail['required'])
                        ...
                        if (Validate::isLoadedObject($step)) {
                            ...
                            if ($selected || (ConfiguratorStepGroupModel::getIdGroupByStepId($step->id) && $this->isSharedStepCompleted($step->id))) {
        ...
        public function isSharedStepCompleted($step_id)
        {
            $step_shared_ids = ConfiguratorStepGroupModel::getLinkedStepById($step_id);
            $detail = $this->getDetail();

            foreach ($detail as $step_detail) {
                if (in_array($step_detail['id'], $step_shared_ids) && $step_detail['id'] !== $step_id
                    && isset($step_detail['required'])
                    && $step_detail['required']
                    && ((isset($step_detail['ignored_if_empty']) && !$step_detail['ignored_if_empty']) || count($step_detail['options']) > 0)
                ) {
                    $step = $this->getConfiguratorStepModel((int) $step_detail['id']);
                    if (Validate::isLoadedObject($step)) {
                        ...
                        if ($selected) {
                            if (isset($this->steps_errors[$step_id])
                                && ($this->steps_errors[$step_id]['key'] === Configurator::ERROR_STEP_REQUIRED
                                    || $this->steps_errors[$step_id]['key'] === Configurator::ERROR_STEP_FILEUPLOAD_REQUIRED)
                            ) {
                                unset($this->steps_errors[$step_id]);
                            }
        ...
        protected function getAmount(ConfiguratorStepOptionAbstract $option, $base_price)
        {
            ...
            // CONFIGURATOR HOOK
            $result = false;
            Hook::exec('configuratorActionCartDetailGetAmount', [
            ...
            $price_list_coeff = ((float) $option->price_list_coeff > 0 ? (float) $option->price_list_coeff : 1);
            ...
            switch ($option->impact_type) {
                ...
                case ConfiguratorStepOptionAbstract::IMPACT_TYPE_PRICELIST_QUANTITY:
                    if ((int) $option->impact_step_id && !empty($option->price_list)) {
                        ...
                        if (!empty($xy_values['x'])) {
                            $this->pricelist_helper->setPricelist(json_decode($option->price_list, true));

                            return ((float) $this->pricelist_helper->getValue($xy_values['x'], $xy_values['y']) * $stepQty) * $price_list_coeff;
                        }
                    } else {
                        if (!empty($option->id_impact_step_option_x) && !empty($option->id_impact_step_option_y)) {
                            $xy_values = $this->findPriceListValuesFromXYStepId(
                ...
                case ConfiguratorStepOptionAbstract::IMPACT_TYPE_PRICELIST_AREA:
                    if ((int) $option->impact_step_id && !empty($option->price_list)) {
                        ...
                        return $surface * $stepQty * $price_list_coeff;
                    }
                                ...
                                if ((int) $id && !empty($option->price_list)) {
                                    ...
                                    if (!empty($xy_values['x']) && !empty($xy_values['y'])) {
                                        $this->pricelist_helper->setPricelist(
                                            json_decode($option->price_list, true)
                                        );

                                        return (float) $this->pricelist_helper->getValue(
                                            $xy_values['x'],
                                            $xy_values['y']
                                        ) * $stepQty * $price_list_coeff;
                                    }
                                }
                            }
                        }
        ...
        public function getPrice(&$step_detail, $base_price)
        {
            ...
            foreach ($step_detail['options'] as &$option_detail) {
                ...
                // CONFIGURATOR HOOK
                $result = false;
                Hook::exec('configuratorActionCartDetailGetPrice', [
                ...
                $option_detail['taxes'][$option->id_tax_rules_group] = [
        ...
        {
            $pricelist = json_decode($step->price_list, true);
            if (empty($pricelist)) {
                return false;
            }
        ...
        public function getTotalStepPrice($step_id)
        {
            ...
            if (isset($detail[(int) $step_id], $detail[(int) $step_id]['total_step_amount'])) {
        ...
        {
            $price_list = json_decode($step->price_list, true);
            $this->pricelist_helper->setPricelist($price_list);
        ...
        public function compute(&$base_product)
        {
            ...
            if (Validate::isLoadedObject($this->product) && Validate::isLoadedObject($base_product)) {
                $detail = $this->getDetail();
                $key = $this->id . '-' . md5(json_encode($detail));
                if (DmCache::getInstance()->isStored($key)) {
                ...
                if ($configurator->use_combination) {
                    $base_product->price = Product::getPriceStatic(
                        $base_product->id,
                        false,
                ...
                foreach ($detail as &$step_detail) {
                    ...
                    // CONFIGURATOR HOOK
                    $stepPriceTaxExclToAdd = Hook::exec('configuratorActionCartDetailComputeAddingPrice', [
                        'cart_detail_model' => &$this,
                        'step' => $step,
                        'detail' => $detail,
                    ]);

                    if ($stepPriceTaxExclToAdd === false || $stepPriceTaxExclToAdd === '' || $stepPriceTaxExclToAdd === null) {
                        if ($step->unique_price) {
                            ...
                            $stepPriceTaxExclToAdd = (float) $this->getPriceFromPricelist(
                                $step_detail,
                                json_decode($step->price_list, true),
                                $base_price,
                    ...
                    $step_detail['delivery_impact'] = $step->getDeliveryImpact($this);
                    if (isset($step_detail['delivery_impact']) && $step_detail['delivery_impact']) {
                        $delivery_impact_hours += (float) $step_detail['delivery_impact'];
                    }
                    ...
                    foreach ($step_detail['options'] as $opt) {
                        if (isset($opt['selected'])
                            ...
                            if (isset($step_detail['multiple'])) {
                                if (!isset($multiple_references[$opt['reference_position']])) {
                                    $multiple_references[$opt['reference_position']] = '';
                                }
                                $multiple_references[$opt['reference_position']] .= $step->getReference($opt['reference'], $this);
                            } else {
                        ...
                        // FIX MIN-MAX
                        /*$option = ConfiguratorStepOptionAbstract::getOption((int)$opt['id']);
                        $step_detail['options'][$opt['id']]['min'] = $this->getMinValue($option);
                        $step_detail['options'][$opt['id']]['max'] = $this->getMaxValue($option);*/
                        ...
                        if (isset($opt['selected']) && $opt['selected'] && isset($opt['delivery_impact']) && $opt['delivery_impact']) {
                            $delivery_impact_hours += (int) $qty * (int) $opt['delivery_impact'];
                        }

                        $configuratorAttribute = new ConfiguratorAttribute($opt['id_option']);
                        if (Validate::isLoadedObject($configuratorAttribute)) {
                            $texture_image = __PS_BASE_URI__ . 'img/co/' . $opt['id_option'] . '.jpg';
                            if (!isset($opt['texture_image']) && (bool) $configuratorAttribute->texture_image) {
                                $step_detail['options'][$opt['id']]['texture_image'] = $texture_image;
                            }
                        }

                        if (!isset($opt['color'])) {
                            $step_detail['options'][$opt['id']]['color'] = ConfiguratorAttribute::getColorByIdOption($opt['id_option']);
                        }
                    }
                    if (isset($step_detail['multiple']) && $step_detail['multiple']) {
                        foreach ($multiple_references as $reference_position => $multiple_reference) {
                            $reference[$reference_position] = $multiple_reference;
                        }
                    }
                }
                ksort($reference);
                ...
                $this->delivery_impact = ceil($delivery_impact_hours / 24);
        ...
        public function taxesCalculation()
        {
            ...
            foreach ($detail as $stepDetail) {
                if (isset($stepDetail['taxes']) && count($stepDetail['taxes']) > 0) {
                    foreach ($stepDetail['taxes'] as $stepDetailTax) {
                        // $tax += $stepDetailTax['price_tax_incl'] - $stepDetailTax['price_tax_excl'];
                        $tax += DMTools::getTaxesFromPriceTaxExcl($stepDetailTax['price_tax_excl'], $stepDetailTax['id_tax_rules_group']);
                    }
                } elseif (isset($stepDetail['price'])) {
                    $tax += $stepDetail['price']['tax_incl'] - $stepDetail['price']['tax_excl'];
                }
            }

            $configurator = new ConfiguratorModel($this->id_configurator);
            $base_price = ($configurator->use_base_price) ? (float) $this->product->price : 0;
            $productTax = DMTools::getTaxesFromPriceTaxExcl($base_price, $this->id_tax_rules_group);
        ...
        public function setCustomization($price = 0, $weight = 0)
        {
            ...
            if (Validate::isLoadedObject($product) && !$product->customizable) {
            ...
            if (Validate::isLoadedObject($customization)) {
                ...
                // CONFIGURATOR HOOK
                Hook::exec('configuratorActionCartDetailModelSetCustomization', [
                    'cart_detail' => $this,
                    'data' => &$data,
                    'price' => &$price,
                ]);
                ...
                $data['price'] = $price;
                $data['weight'] = $weight;
                $data['id_module'] = Module::getModuleIdByName('configurator');
        ...
        public function generateCustomizedDatas()
        {
            ...
            // CONFIGURATOR HOOK
            Hook::exec('configuratorActionCartDetailModelGenerateCustomizedDatas', [
                'cart_detail' => $this,
                'return' => &$return,
            ]);
        ...
        public function getDetail($unset_empty_step = false)
        {
            if ($this->detailJSON === false) {
                $this->detailJSON = json_decode($this->detail, true);
            }
            $detail = (empty($this->detailJSON) ? [] : $this->detailJSON);
        ...
        public function getDetailFormated(
            ...
            foreach ($detail as $step_detail) {
                ...
                if ($step_detail['is_upload'] && $step_detail['file_has_been_uploaded']) {
                    ...
                    foreach ($step_detail['options'] as $option_detail) {
                        if ($option_detail['value'] === '0' || (!empty($option_detail['value']) && $option_detail['value'] !== false)) {
                ...
                if ($display) {
                    ...
                    $list_detail[] = (
                        $text
                            ? $step_name . ' <span class="configurator_separator">-</span> ' // Soucis avec certains thèmes lorsque le séparaeur est " : "
                    ...
                    ) . implode($separator_option, $array_options) . (
                        $text
                        ? ''
                        : '</span></span>'
                    );
        ...
        public function getStepQuantities()
        {
            $stepQuantities = 0;
            $detail = $this->getDetail();
            foreach ($detail as $step_detail) {
                if (!$step_detail['is_upload']) {
                    foreach ($step_detail['options'] as $option_detail) {
                        if ($option_detail['selected'] && $step_detail['use_qty'] && $step_detail['unit_qty_step']) {
                            $stepQuantities += (int) $option_detail['qty'];
                        }
                    }
                }
            }
         ...
         */
        public function foundOption($value, $check_selected = true, $check_value = false, $accept_zero_value = false)
        {
            $detail = $this->getDetail();
            foreach ($detail as $step_detail) {
                if (isset($step_detail['options']) && $step_detail['options']) {
                    foreach ($step_detail['options'] as $option_detail) {
                        if ((int) $value === (int) $option_detail['id']) {
                            if (!$check_selected && !$check_value) {
                                return true;
                            }
                            if ($check_selected && $option_detail['selected']) {
                                return true;
                            }
                            if ($check_value && $option_detail['value']) {
                                return true;
                            }
                            if ($check_value && $accept_zero_value && $option_detail['value'] === '0') {
                                return true;
                            }
                        }
                    }
        ...
        public function addStep(ConfiguratorStepAbstract $step, $extras = [])
        {
            $detail = $this->getDetail();
            ...
            $detail_resorted[(int) $step->position] = [
                'id' => (int) $step->id,
                ...
                'unit_qty_step' => $step->unit_qty_step,
            ...
            // CONFIGURATOR HOOK
            Hook::exec('configuratorActionCartDetailAddStep', [
                'cart_detail_model' => &$this,
        ...
        public function addOption($id_step, $configurator_step_option, $extras = [], $erase_selection = false, $dropzone_positions = null)
        {
            /**
            ...
            if ($step->use_qty) {
                ...
                $min_qty = $this->getMinQty($configurator_step_option);
                $max_qty = $this->getMaxQty($configurator_step_option);
                $extras['selected'] = !isset($extras['qty']) ? (bool) ($default_qty > 0) : $extras['selected'];
                if ($configurator_step_option->force_value) {
                    $extras['qty'] = $default_qty;
                } else {
                    $extras['qty'] = !isset($extras['qty']) ? $default_qty : $extras['qty'];
                }
                if ($min_qty && $extras['qty'] < $min_qty) {
                    $extras['qty'] = $min_qty;
                } elseif ($max_qty && $extras['qty'] > $max_qty) {
                    $extras['qty'] = $max_qty;
                }
            }
            ...
            if (Validate::isLoadedObject($configurator_step_option)
                ...
                if ($dropzone_positions !== null && isset($detail[(int) $id_step], $detail[(int) $id_step]['dropzone'])) {
                ...
                $detail[(int) $id_step]['options'][(int) $configurator_step_option->id] = [
                    ...
                    'delivery_impact' => (int) $configurator_step_option->delivery_impact,
                ...
                // CONFIGURATOR HOOK
                Hook::exec('configuratorActionCartDetailAddOption', [
                    'cart_detail_model' => &$this,
                    'detail' => $detail,
            ...
            if (Module::isInstalled('dm_advancedformula') && Module::isEnabled('dm_advancedformula')) {
                if ($configuratorStep !== null) {
                    $max_value = AdvancedformulaHelper::loadFormula(
                        ...
                        $max_value,
                        'float',
                        'Option #' . $configurator_step_option->id . ' max_value'
                    );
            ...
            if (Module::isInstalled('dm_advancedformula') && Module::isEnabled('dm_advancedformula')) {
                if ($configuratorStep !== null) {
                    $min_value = AdvancedformulaHelper::loadFormula(
                        ...
                        $min_value,
                        'float',
                        'Option #' . $configurator_step_option->id . ' min_value'
                    );
        ...
        public function getDefaultQty($configurator_step_option)
        {
            ...
            if (Module::isInstalled('dm_advancedformula') && Module::isEnabled('dm_advancedformula')) {
                if ($configuratorStep !== null) {
                    $default_qty = AdvancedformulaHelper::loadFormula(
                        $this,
                        $configuratorStep,
                        $this->getDetail(),
                        $default_qty,
                        'float',
                        'Option #' . $configurator_step_option->id . ' default_qty'
                    );
                }
            }
        ...
        public function getMinQty($configurator_step_option)
        {
            $min_qty = $configurator_step_option->option_min_qty;
            $configuratorStep = $this->getConfiguratorStepModel($configurator_step_option->id_configurator_step);

            if (Module::isInstalled('dm_advancedformula') && Module::isEnabled('dm_advancedformula')) {
                if ($configuratorStep !== null) {
                    $min_qty = AdvancedformulaHelper::loadFormula(
                        $this,
                        $configuratorStep,
                        $this->getDetail(),
                        $min_qty,
                        'float',
                        'Option #' . $configurator_step_option->id . ' min_qty'
                    );
                }
            }
        ...
        public function getMaxQty($configurator_step_option)
        {
            $max_qty = $configurator_step_option->option_max_qty;
            $configuratorStep = $this->getConfiguratorStepModel($configurator_step_option->id_configurator_step);

            if (Module::isInstalled('dm_advancedformula') && Module::isEnabled('dm_advancedformula')) {
                if ($configuratorStep !== null) {
                    $max_qty = AdvancedformulaHelper::loadFormula(
                        $this,
                        $configuratorStep,
                        $this->getDetail(),
                        $max_qty,
                        'float',
                        'Option #' . $configurator_step_option->id . ' max_qty'
                    );
                }
            }
         ...
         * @param bool $remove unset option or just unselect it
         */
        public function removeOption($value, $remove = true, $extra = [])
        {
            $detail = $this->getDetail();
            $oldDetail = $detail;
            foreach ($detail as &$step_detail) {
        ...
        public function checkMinMaxValue($value, $configurator_step_option)
        {
            ...
            if ($configurator_step_option->textarea) {
                $configurator_step = ConfigurationStepFactory::newObject($configurator_step_option->id_configurator_step);
                if (strpos($configurator_step->class, 'no-space-textarea') !== false) {
        ...
        public function computeOptions($detail, $oldDetail, ConfiguratorStepAbstract $configuratorStep, $extras)
        {
            ...
            if ($extras['multiple']) {
                ...
                if ($count > 0 && $configuratorStep->min_options > $count) {
                    $this->steps_errors[$configuratorStep->id] = [
         ...
         * @return float|int
         */
        public static function getProductDimension($type = '', $cart = null, $product = null)
        {
        ...
        public static function getMaxDelayInCart()
        {
            $maxDeliveryImpact = 0;
            if (isset(Context::getContext()->controller->configuratorCartDetail) && Validate::isLoadedObject(Context::getContext()->controller->configuratorCartDetail)) {
                $maxDeliveryImpact = (int) Context::getContext()->controller->configuratorCartDetail->delivery_impact;
            } else {
                $cartDetails = ConfiguratorCartDetailModel::getByIdCart((int) Context::getContext()->cart->id);
                foreach ($cartDetails as $cartDetail) {
                    if ($maxDeliveryImpact < $cartDetail->delivery_impact) {
                        $maxDeliveryImpact = (int) $cartDetail->delivery_impact;
                    }
                }
            }

            return $maxDeliveryImpact;
        }

        public function duplicate()
        {
            $newCartDetail = new ConfiguratorCartDetailModel();
            $newCartDetail->id_configurator = $this->id_configurator;
            $newCartDetail->id_product = $this->id_product;
            $newCartDetail->id_product_attribute = $this->id_product_attribute;
            $newCartDetail->id_guest = 0;
            $newCartDetail->id_cart = 0;
            $newCartDetail->id_order = 0;
            $newCartDetail->id_order_detail = 0;
            $newCartDetail->detail = $this->detail;
            $newCartDetail->added_in_cart = 0;
            $newCartDetail->price = $this->price;
            $newCartDetail->weight = $this->weight;
            $newCartDetail->delivery_impact = $this->delivery_impact;
            $newCartDetail->id_customization = 0;
            $newCartDetail->id_tax_rules_group = $this->id_tax_rules_group;
            $newCartDetail->reference = $this->reference;
            $newCartDetail->is_shared = 0;
            $newCartDetail->save();
